...
const signUp = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: `Sign up was successful, please log in to continue`, newUser});      
    } catch (error) {
    console.log(error)
  }
};
...
const logIn = async (req, res) => {
  try {
    ...
    if (!user || !user.authenticate(password)) {
      return res.status(400).json({ error: "User name and password do not match."})
    }
