...
const getRecipes = async (req, res) => {
  try {
    const { username, viewer, search } = req.query;
    ...
    if (username) {
      if (username === viewer) {
        ...
        totalRecipes = recipes.length;
        recipes = await Recipe.find({ createdBy: username, share: true }).sort(
          "-updatedAt"
        );
      }
    } else {
  ...
  try {
    ...
    for (let recipeId of user.liked_recipes) {
      ...
      if (!recipe) {
        ...
        const newLikes = [
          ...user.liked_recipes.slice(0, index),
          ...user.liked_recipes.slice(index + 1),
        ];
...
const updateRecipe = async (req, res) => {
  try {
    ...
    if (check.createdBy !== req.user.username) {
      return res
        .status(401)
        .json({ msg: "Only the recipe's creator can update it." });
    }
    const newRecipe = { name, time, ingredients, directions, image, share };
    if (
      image.url !== check.image.url &&
      image.url !== "/food-placeholder.png"
    ) {
...
const deleteRecipe = async (req, res) => {
  try {
    ...
    if (check.createdBy !== req.user.username) {
      return res
        .status(401)
        .json({ msg: "Only the recipe's creator can delete it." });
    }
    if (check.image.url !== "/food-placeholder.png") {
      await cloudinary.uploader.destroy(
        check.image.id,
