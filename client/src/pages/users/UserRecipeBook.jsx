  ...
  if (!recipes) {
    return (
      <div className='container d-flex justify-content-center align-items-center' style={{height: "75vh", width: "100vw"}}>
        <h2><span className='text-primary'>{user.username === creator ? "You have" : `${creator} has`}</span> no recipies yet</h2>
      </div>
    )
  }
  ...
  return (
    <div className="container-lg">
      <div className="container-fluid d-flex flex-column flex-sm-row justify-content-between">
        <h2 className='text-muted h1 ms-5 py-2' style={{fontFamily: "Permanent Marker, cursive"}}><span className='fst-italic text-primary'>{user.username === creator ?  "Your" : `${creator}'s`}</span>{ homeView ? " recipes" : " favorited recipes"}</h2>
        {user.username === creator && (
          <div className='col-12 col-sm-3 d-flex align-items-center'>
            <button type="button" onClick={handleViewClick} className='btn btn-primary mx-auto' style={{height: "50%"}}>{homeView && (<i className='bi bi-heart'></i>)} {homeView ? " Favorites" : "My Recipes"}
            </button>
          </div>
        )}
      ...
      <div className="container-fluid d-flex justify-content-center flex-wrap gap-4 mt-4">
        {homeView ? recipes.map( (recipe) => <RecipeCard key={recipe._id} recipe={recipe} username={user.username} />) : likes.map((recipe) => <RecipeCard key={recipe._id} recipe={recipe} username={user.username} /> )}     
      </div>
    </div>
  )
}
